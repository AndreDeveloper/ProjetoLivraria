...
import edu.livraria.control.ClienteController;
...
public class CadastroClienteBoundary implements ActionListener {
	...
	public Cliente EventoConcluirCadastro() {
...
//		clt.setCpf(cpf.getText());
	...
	public boolean ValidaCampos() {
		...
		if (nome.getText().length() <= 0) {
			...
			cidade.requestFocus();
			return false;
		} else if (cbSexo.getSelectedItem().toString().equals("")){
			...
			return false;
				...
				.length() <= 0 || telefone.getText().replace("-", "").replace("(", "").replace(")", "").trim().length() < 10) {
			...
			return false;
				...
				.length() <= 0 ||  celular.getText().replace("-", "").replace("(", "").replace(")", "").trim().length()<11) {
